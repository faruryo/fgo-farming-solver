...
const ProgressBar = ({ row }: { row: Row }) => {
  ...
  const allocPct =
    (Math.min(row.allocated, Math.max(0, denom - row.owned)) / denom) * 100
  ...
  return (
    ...
    >
      ...
      />
      <div
        style={{ width: `${shortPct}%`, background: 'rgba(176,48,48,0.35)' }}
      />
    </div>
  )
}
...
export const MaterialSelectionAdvisor = ({
  ...
  const dropItemByAtlasId = useMemo(() => {
    ...
    for (const it of drops.items)
      if (it.atlasId != null) m.set(String(it.atlasId), it)
    return m
  }, [drops.items])
  ...
  const bufferFor = useCallback(
    (id: string): number => {
      ...
      const itemLike = itemsById.get(id) ?? dropItemByAtlasId.get(id)
      return itemLike
        ? buffer(toStockItemLike(itemLike), resolvedStockBuffer)
        : 0
    },
  ...
  const fullNeed = useMemo(
    ...
    [amounts, possession, drops, resolvedStockBuffer, purpose],
  )
  ...
  const { rows, totalSaved, totalAllocated, top } = useMemo(() => {
    ...
    const rows: Row[] = candidateRefs.map((c, i) => {
      ...
      return {
        ...
        required:
          purpose === 'reserve'
            ? Math.max(trainingRequired, stockBuffer)
            : trainingRequired,
        trainingRequired,
        stockBuffer,
        owned,
    ...
    const top = rows
      ...
      .reduce<Row | null>(
        (best, r) => (best == null || r.saved > best.saved ? r : best),
        null,
      )
    return {
      rows,
    ...
    return items
      ...
      .sort((a, b) => {
        const rank = (x: typeof a) =>
          x.deficiency > 0 ? 0 : x.required > 0 ? 1 : 2
