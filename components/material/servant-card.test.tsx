...
describe('ServantCard - ascension pips', () => {
  ...
  it('does not go below the ascension minimum (0) via the long-press/right-click decrement', () => {
    const onStartChange = vi.fn()
...
describe('ServantCard - long press / right click decrement', () => {
  ...
  it('a long press (>=500ms) on a pip decrements start by 1 and suppresses the following click', () => {
    const onStartChange = vi.fn()
    const state = ownedState()
    state.targets.ascension.ranges[0] = { start: 2, end: 4 }
    const { setState } = renderCard(state, { onStartChange })
    ...
    const pips = document.querySelectorAll('.c-sum-pip')
    fireEvent.pointerDown(pips[1])
    vi.advanceTimersByTime(500)
    fireEvent.pointerUp(pips[1])
    // Browsers fire a click after pointerup; the long-press flag should
    // swallow it so it doesn't also apply the click-cycle logic.
    fireEvent.click(pips[1])
