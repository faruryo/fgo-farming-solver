...
import { computeQuestEfficiency, mergeGoals } from '../../lib/quest-efficiency'
import {
...
const CheckLabel: React.FC<{
  ...
  >
    <Checkbox
      checked={checked}
      onCheckedChange={(c: boolean) => onChange(Boolean(c))}
    />
    {children}
  </label>
)
...
const filterRowLabelClass =
  'text-[10px] font-semibold tracking-wide w-16 flex-shrink-0'
...
export const QuestEfficiencyList: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return ranked.filter(
      (r) =>
        r.quest.name.toLowerCase().includes(q) ||
        r.quest.area.toLowerCase().includes(q),
    )
  }, [ranked, query])
  ...
  const unenteredCount = useMemo(() => {
    ...
    return dropItems.filter(
      (i) =>
        dropItemIds.has(i.id) &&
        i.atlasId != null &&
        possession[String(i.atlasId)] == null,
    ).length
            ...
            return (
              ...
              >
                ...
                <div className="hidden sm:flex flex-1 items-center gap-1 min-w-0 overflow-hidden">
                  {(dropItemsByQuest.get(r.quest.id) ?? [])
                    .slice(0, 8)
                    .map((it) => (
                      <ItemIdentity
                        key={it.id}
