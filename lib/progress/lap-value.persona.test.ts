...
describe('ペルソナ受け入れテスト(design.md、elapsedMinutes=43200基準)', () => {
  it('P1 新米: 低率クエストしか選択していなくても自然APフル相当(≈19周/日)で medium に到達する(2026-07-18 しきい値改定で large 基準から変更)', () => {
  ...
  it('P3 ボックス月: 銅銀素材の大量不足解消で≈110周/日 → legendary', () => {
    const drops = makeDrops([
    ...
    const targets = { '6006': 3000, '6007': 3000 }
    const past = { '6006': 0, '6007': 0 }
    const current = { '6006': 1000, '6007': 1300 }
    ...
    // f: 1000×2=2000周。g: 1300×1=1300周。合計3300周/30日=110周/日。
    expect(forward!.forwardLaps).toBeCloseTo(3300)
    expect(forward!.forwardLaps / (ELAPSED_30_DAYS / 1440)).toBeCloseTo(110)
