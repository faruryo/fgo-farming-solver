...
describe('computeQuestEfficiency', () => {
  it('bestEff で正規化される(最良クエストは relativeEff=1、効率半分は 0.5)', () => {
    const res = byId(
      computeQuestEfficiency(makeDrops(), { shortageOnly: false }),
    )
  ...
  it('不足(owned < goal)は重み1', () => {
    const res = byId(
      computeQuestEfficiency(makeDrops(), { goals: { g: 10 }, possession: {} }),
    )
  ...
  it('includeQp で QP を擬似アイテムとして加算する(default は加算しない)', () => {
    ...
    expect(off.qA.contributions.some((c) => c.itemId === 'reward:qp')).toBe(
      false,
    )
    ...
    expect(
      on.qB.contributions.find((c) => c.itemId === 'reward:qp')?.relativeEff,
    ).toBeCloseTo(0.5)
    // 加算により score が上がる
...
describe('computeItemWeight: stockEnabled の ON/OFF と境界(D2)', () => {
  ...
  it('育成不足(owned<goal)は stockEnabled に関わらず重み1', () => {
    expect(
      computeItemWeight(goldMat, 5, 10, { ...baseOpts, stockEnabled: false }),
    ).toBe(1)
    expect(
      computeItemWeight(goldMat, 5, 10, { ...baseOpts, stockEnabled: true }),
    ).toBe(1)
  })
  ...
  it('OFF: owned==effGoal(goal+buffer)は境界外(重み0)', () => {
    ...
    expect(
      computeItemWeight(goldMat, 60, 10, { ...baseOpts, stockEnabled: false }),
    ).toBe(0)
  })

  it('OFF: owned==effGoal-1 は次点(0.3)', () => {
    expect(
      computeItemWeight(goldMat, 59, 10, { ...baseOpts, stockEnabled: false }),
    ).toBeCloseTo(SECONDARY_WEIGHT)
  })

  it('ON: owned==effGoal-1 は目標(1.0)に昇格', () => {
    expect(
      computeItemWeight(goldMat, 59, 10, { ...baseOpts, stockEnabled: true }),
    ).toBe(1)
  })

  it('ON: owned==effGoal は重み0(ストック込み目標に到達)', () => {
    expect(
      computeItemWeight(goldMat, 60, 10, { ...baseOpts, stockEnabled: true }),
    ).toBe(0)
  })
  ...
  it('buffer=0(モニュピ銅相当の未設定レア)は OFF/ON 双方で owned>=goal なら重み0', () => {
    ...
    expect(
      computeItemWeight(goldMat, 10, 10, {
        ...baseOpts,
        stockBuffer,
        stockEnabled: false,
      }),
  ...
  it('goal=0 かつ stockEnabled=ON は effGoal=buffer になり、所持がそれ未満なら重み1', () => {
    ...
    expect(
      computeItemWeight(goldMat, 49, 0, { ...baseOpts, stockEnabled: true }),
    ).toBe(1)
    expect(
      computeItemWeight(goldMat, 50, 0, { ...baseOpts, stockEnabled: true }),
    ).toBe(0)
  })

  it('goal=0 かつ stockEnabled=OFF は次点(0.3)で低所持を拾う(従来挙動)', () => {
    expect(
      computeItemWeight(goldMat, 49, 0, { ...baseOpts, stockEnabled: false }),
    ).toBeCloseTo(SECONDARY_WEIGHT)
  })

  it('レア不明の素材はストック対象外(buffer=0)で owned>=goal なら重み0', () => {
    expect(
      computeItemWeight(unknownRarity, 0, 0, {
        ...baseOpts,
        stockEnabled: true,
      }),
    ).toBe(0)
  ...
  it('カテゴリ群でストック個数が異なる(通常素材 gold=50 / スキル石 gold=60)', () => {
    ...
    expect(
      computeItemWeight(goldMat, 55, 0, { ...baseOpts, stockEnabled: true }),
    ).toBe(0)
    expect(
      computeItemWeight(goldGem, 55, 0, { ...baseOpts, stockEnabled: true }),
    ).toBe(1)
  })
})

describe('周回目的別の低在庫評価', () => {
  const opts = {
    includeSkillStones: true,
    includePieces: true,
    stockBuffer: BUF,
  ...
  it('必要総数ではなく所持数の少ない素材を優先する', () => {
    const manyOwned = computeItemWeight(goldMat, 200, 1000, {
      ...opts,
      purpose: 'training',
    })
    const fewOwned = computeItemWeight(goldMat, 30, 100, {
      ...opts,
      purpose: 'training',
    })
    expect(fewOwned).toBeGreaterThan(manyOwned)
  })

  it('倍率は1以上2以下で、buffer=0なら1', () => {
    expect(
      computeItemWeight(goldMat, 0, 1, { ...opts, purpose: 'training' }),
    ).toBe(2)
    expect(
      computeItemWeight(unknownRarity, 0, 1, { ...opts, purpose: 'training' }),
    ).toBe(1)
  })

  it('reserveはrequiredとbufferの大きい方を有限目標にする', () => {
    expect(computeFiniteTarget(goldMat, 10, BUF, 'reserve')).toBe(50)
    expect(computeFiniteTarget(goldMat, 100, BUF, 'reserve')).toBe(100)
  })

  it('allは全素材を一律評価する', () => {
    expect(
      computeItemWeight(goldMat, 999, 0, { ...opts, purpose: 'all' }),
    ).toBe(1)
  })
})
...
describe('effectiveDeficiency: 共有純関数の境界(クエスト効率の重み判定と整合)', () => {
  ...
  it('クエスト効率の重み判定(computeItemWeight)と同じ境界で一致する', () => {
    ...
    expect(
      effectiveDeficiency(goldMat, goal, effGoalMinus1, BUF, true),
    ).toBeGreaterThan(0)
    expect(
...
describe('buildNeedByApiItemId', () => {
  ...
  it('reserveではrequiredとbufferの大きい方を目標にする', () => {
    const need = buildNeedByApiItemId(
      { '1': 10 },
      { '1': 0, '2': 100 },
      dropsWithAtlas,
    ...
    // gold(normal) buffer=50 → max(required 10, buffer 50)
    // silver(normal) 目標未設定(0)・所持100 = buffer(100) なので effGoal=100, 不足0
    expect(need).toEqual({ g: 50 })
  })
  ...
  it('allの数量計算はtrainingへ戻し、reserveだけ在庫基準を使う', () => {
    const targets = { '1': 10 }
    const possession = { '1': 4 }
    expect(
      buildNeedByApiItemId(targets, possession, dropsWithAtlas, BUF, 'all'),
    ).toEqual({ g: 6 })
    expect(
