...
const run = (
  overrides: Partial<Parameters<typeof runMaterialCatalogPhase>[0]> = {},
) =>
...
describe('runMaterialCatalogPhase', () => {
  ...
  it.each([
    ...
    'keeps last-known-good when Atlas supplies %s',
    async (_description, invalidServant) => {
      const put = vi.fn().mockResolvedValue(undefined)
      const fetchSource = vi
        .fn()
        .mockResolvedValueOnce(
          Response.json([invalidServant()], {
            headers: { etag: 'servant-v2' },
          }),
        )
        .mockResolvedValueOnce(
          Response.json(items, { headers: { etag: 'item-v1' } }),
        )
  ...
  it('reports failure when the one-key KV write fails', async () => {
    ...
    const fetchSource = vi
      .fn()
      .mockResolvedValueOnce(
        Response.json([servantWithMaterials], {
          headers: { etag: 'servant-v2' },
        }),
      )
      .mockResolvedValueOnce(
        Response.json(items, { headers: { etag: 'item-v2' } }),
      )

    await expect(
      run({ get: vi.fn().mockResolvedValue(null), put, fetchSource }),
