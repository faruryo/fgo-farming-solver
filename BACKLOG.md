...
## 素材計算から周回数計算への遷移における数値相違

**重要度: 高（データ引き継ぎの正確性）**

### 問題

素材計算（サーヴァント選択）→「必要な素材を計算する（`/material/result`）」→「周回数を求める（`/farming`）」と遷移した際、ストック目標を有効にしていても引き継がれる素材数の数字が異なっている。

### 原因

`components/material/result.tsx` の `goSolver` から `/farming` へ渡る目標A(`items`)は `useLocalStorage` で永続化されるのに対し、目標B(`itemsStock`)は `/farming` 側の揮発性 state にしか保持されない。取り込み後に `/farming` の入力欄を手編集すると、編集済みの目標Aと取り込み時のまま古い目標Bが対応しない状態で計算履歴に保存される。
...
（検討の途中で「`/farming` 側で手編集を検知して目標Bを破棄するフォールバック」案を一度proposeしたが、上記の方が根本的なため取り下げた。）

### 見送った案

「`/material/result` を飛ばして素材計算画面から直接 `/farming` へ移動するショートカット」も検討したが、実際の使い方として所持アイテム数の調整を `/material/result` 側で行っており、単に確認して次へ進むだけのクッションページではない。飛ばすケース自体があまり無いため見送り。
