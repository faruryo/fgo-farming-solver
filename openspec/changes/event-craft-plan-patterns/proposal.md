...
- 5本とも、適用時点の料理ソルバーと同じ獲得yield（期待値マップがあればそれ）を使う。期待値テーブル自体の導入は `event-craft-expected-yields` のまま。
...
- 期待値テーブルの新規定義は `event-craft-expected-yields` の範囲。本変更はパターン間でyieldモデルを分岐させない。
