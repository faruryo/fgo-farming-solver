...
3. 不足／余剰は **その使い切り配分の中** で分ける。`runs` の個数との差では分けない（runs が A のみ、exhaust が B のみでも、B が残余コストを下げていれば不足枠）。
   - 同一レシピの中で不足充足に効く個数と純粋な余剰個数が混在し得るため、レシピ全体のゼロ化では1個の不足も9個の余剰もまとめて「不足あり」に丸めてしまう。そこで **0個から実際の作成数まで1個ずつ段階的に** 残余周回コストを評価し、実際の作成数と同じ残余周回コストに最初に達する個数を不足枠、残りを余剰枠にする。残余周回コストは個数を増やすほど単調非増加なので線形走査で決まる。
