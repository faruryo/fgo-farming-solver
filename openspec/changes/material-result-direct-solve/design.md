...
### 目標Aが0件(stock-only)のときは目標Bを単独送信する
`queryItemsA`が空文字のまま`itemsStock=`付きで送信すると、`/api/solve`の`hasBatch`判定が真になり2目標バッチ扱いになる。バッチの着地先(`id`)は常に目標A行であるため、目標Aが空だと0件・0APの空虚な結果ページに着地してしまう。これを避け、目標Aが空のときは目標Bを唯一の`items`として単独送信する(`itemsStock`を付けずバッチにしない)。この経路では`stockIncluded=true`が付与されない(付与は2目標バッチ保存時のみの既存契約)ため、結果画面の「ストック込み」バッジはこのケースで出ない。stock-onlyの発生頻度は低く、バッジ欠落は表示上の軽微な影響にとどまるため許容する。

- 代替案: `/api/solve`側にA=0時のstockIncluded単独付与を追加する。→ 却下。サーバ側contractの変更は本changeのNon-Goal(目標A/B算出アルゴリズム・保存契約の変更)に抵触し、スコープが拡大する。
