...
- [ ] 3.1 純関数でおすすめ皿数から育成素材ごとの期待獲得合計を出す（作成数×yield の和を加算、上書き禁止、QP/種火除外）。単一銅 N 皿に加え、yield が重なる複数料理（不足枠と余剰枠の両方）で合計が足し算になるケースをテストする
- [ ] 3.2 結果画面におすすめ割り振りと並んで「この配分での期待獲得」を `t()` で出す（日英 locale）。コンポーネントテストは見出しと、加算済みの期待個数を少なくとも1つ明示的に検証する（料理カードの1皿内訳だけでは合格にしない）
