...
- [x] 9.7 本番マスターデータ(`update.ts`)にも `atlasId` をベイクし、旧データ向けに `getDrops` で欠損分のみ実行時補完(`get-drops.test.ts` 追加)。所持数モーダルが material/result と確実に連動

## 10. クエスト報酬表示・一覧レイアウト調整

- [x] 10.1 クエスト詳細に報酬(QP/基本絆P/EXP)を常時表示する報酬カードを追加(加算トグルとは独立)。i18n 追加
- [x] 10.2 一覧の効率ポイントを右端へ右寄せし、SP(アイコン非表示時)でクエスト名の幅を確保。sm 以上は従来レイアウト維持。ブラウザで SP/PC 両方を確認
