...
1. 余り3種合計の最小化。
2. その制約で層②の周回コスト最小。
3. 不足／余剰は **その使い切り配分の中** で分ける。`runs` の個数との差では分けない（runs が A のみ、exhaust が B のみでも、B が残余コストを下げていれば不足枠）。
   - 同一レシピの中で不足充足に効く個数と純粋な余剰個数が混在し得るため、レシピ全体のゼロ化では1個の不足も9個の余剰もまとめて「不足あり」に丸めてしまう。そこで残余周回コストが個数を増やすほど単調非増加であることを利用し、**二分探索で** 実際の作成数と同じ残余周回コストに達する最小個数を求め、それを不足枠、残りを余剰枠にする（O(log n)。所持食材が多いときの線形走査によるLP解決回数の爆発とUI固まりを避ける）。
