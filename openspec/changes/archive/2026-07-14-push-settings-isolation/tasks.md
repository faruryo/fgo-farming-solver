...
- [x] 2.4 購読解除 API の失敗時に再試行を促し、ブラウザ側の購読状態と D1 の登録状態が不一致になった場合に復旧できることを検証する。（2026-07-15 実機検証: セッションcookie削除で DELETE を401にし、専用トースト表示・トグルON維持・D1行残存の不整合を再現。再ログイン→OFF→ONで新購読が再POSTされD1に登録、復旧を確認。旧行は次回バッチ送信時の410掃除待ち）
...
- [x] 4.3 意図的にAPI送信を失敗させる、または通知許可を拒否した際に、改善された案内メッセージが正しくトースト表示されることを確認する。（2026-07-15 実機検証: 401経路で unsubscribe-failed の専用トーストを確認。既知ギャップ: fetch自体のネットワーク断は TypeError となり PushSubscribeError を経由しないため汎用トーストになる）
