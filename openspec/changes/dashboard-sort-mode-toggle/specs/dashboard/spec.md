...
- **THEN** 各アイテムについて `Math.ceil(needed / drop_rate)` が最小となるクエストを 1 つ選び、選ばれたクエストの周回数昇順でアイテムを並べる。

#### Scenario: AP モードでのクエスト選定
- **WHEN** ソートモードが `AP` のとき
- **THEN** 各アイテムについて `quest.ap / drop_rate` (1 個あたりの実効 AP) が最小となるクエストを 1 つ選び、選ばれたクエストの周回数昇順でアイテムを並べる。
