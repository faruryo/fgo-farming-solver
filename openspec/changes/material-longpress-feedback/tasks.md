...
- [x] 1.2 再臨ピップ・スキルチップ・アペンドチップの `className` に、押下中のみ `is-pressing` を付与する。
- [x] 1.3 `app/globals.css` に `.c-sum-pip.is-pressing` / `.c-sum-card.is-pressing` を追加し、軽い scale 縮小 + 明度低下 + インセットシャドウで押下感を表現する。既存の `.c-sum-card:active` とは共存させる。
...
- [ ] 2.4 実機（ブラウザ）で `/material` の育成記録モード ON/OFF 双方において、ピップ／チップの長押し中に押下フィードバックが表示され、途中でポインタを離す・カード外に出すと即座に元に戻ることを確認する（マージ前必須、本 PR 作成時点では未実施）。
