...
システムは `/quests` にクエスト一覧/検索ページを提供 SHALL。各クエストのエリア・名前・AP(effectiveAP と元 AP)・効率ポイントを表示し、名前/エリアでの検索、効率ポイント降順ソート、「石含む/除く」「不足のみ/全部」トグルを提供 SHALL。ストームポッド消費クエストとポッド無料中クエストにはバッジを表示 SHALL。効率ポイントの意味を説明する導線をメイン行に常設し、説明文はクエスト詳細ページと共通のものを用いる SHALL。スコア見出しは選択中の分母を含み（AP効率ポイント / 周回効率ポイント）、文字色で分母を区別 SHALL。
...
システムは `/quests/[id]` 詳細に当該クエストの効率ポイントと素材別 contribution(relativeEff × 重み)の内訳を表示 SHALL。効率ポイントの見出しには説明ツールチップを付け、一覧ページと同じ説明文を参照する SHALL。見出しは選択中の分母を含み（AP効率ポイント / 周回効率ポイント）、文字色で分母を区別 SHALL。見出し横には分母トグル（AP効率 / 周回効率）を配置し、詳細画面のままでも切り替えられる SHALL。内訳リストには各列(素材・重み・寄与度)が何を表すかを示す見出しを表示 SHALL。
...
- **THEN** スコア0.00および「対象となる不足素材がありません」が表示され、分母トグルを操作できる
