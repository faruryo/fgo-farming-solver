...
### Requirement: 所持アイテム数管理とスクショ取込
ユーザーはダッシュボード上から所持アイテム数の確認・編集およびスクリーンショット画像による一括取込（OCR）を直接呼び出せなければならない (SHALL)。

#### Scenario: 所持素材編集モーダルの起動
- **WHEN** ユーザーがヘッダーの「所持アイテム」ボタンまたは NearGoalSection の「所持数を編集」ボタンをクリックしたとき
- **THEN** 全アイテムのカテゴリ別所持数一覧を編集できるモーダル (`PossessionModal`) が表示される。
- **THEN** モーダル内からスクリーンショット読込ダイアログ (`PossessionImportDialog`) を起動でき、OCR解析結果を所持数に一括反映できる。
- **THEN** 更新された所持数は `localStorage` の `posession` キーに保存され、ダッシュボード内の関連セクション（NearGoalSection 等）に即時反映される。
...
#### Scenario: TODO完了済みイベントの折りたたみ・非表示制御
- **WHEN** ユーザーが該当イベントのアイテム交換TODOタスクを完了（チェック済み）にしているとき
- **THEN** 「完了済みを非表示」設定が有効な場合、該当イベントカードはメイン表示から除外され、セクション下部のアコーディオン等から展開可能となる。
- **THEN** 「完了済みを非表示」の設定状態は `localStorage` の `dashboard.eventSection.hideCompleted` キーに永続化される。
- **THEN** TODOタスク状態の判定は読取専用で行われ、`todoState` への意図しないタスク追加や書き込み副作用は発生しない。
...
- **WHEN** 未完了のデイリーミッション、ウィークリーミッション、イベント交換、またはカスタム TODO が存在し、それらの期限が閾値（デイリー: 24時間以内、ウィークリー/イベント/カスタム: 48時間以内）未満のとき
