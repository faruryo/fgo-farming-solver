...
- **全体共通の目標保持**: `chaldeaState.all` キーに共通目標を保持する。個別データを持たないサーヴァントの `targets` を組み立てる際、`end` のみを `all` から継承する（共通目標パネルの操作により継続的に更新される）。`start` は `all` から継承せず、常に正しいデフォルト値が使われる。`chaldeaState.all.targets.*.start` を編集する手段は UI 上に存在しないため、この値自体は恒久的に凍結されたまま残るが、他のサーヴァントの表示・計算には一切使われない。未所持サーヴァントの現在値（`start`）はマージ時に矯正されない（保存されていた値がそのまま維持される）。
