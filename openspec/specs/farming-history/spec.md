...
システムは、ログインユーザー自身の有効な（削除されていない）計算履歴を新しい順に一覧表示しなければならない (SHALL)。日時は D1 の UTC 記録を正確に解釈し、閲覧端末（クライアント）のローカルタイムゾーンで整形表示しなければならない (SHALL)。
...
- **WHEN** 履歴一覧の日時を表示するとき
- **THEN** SQLite 由来の UTC 日時文字列（`YYYY-MM-DD HH:MM:SS`）または ISO 8601 文字列が確実に UTC として解釈され、クライアント端末のローカルタイムゾーンに変換された日時が表示される。
...
システムは、計算履歴推移グラフ（`FarmingHistoryChart`）における期間フィルタリング、回帰直線計算、X 軸目盛り、ツールチップ表示において、各履歴行の保存日時（UTC）を正確に解釈し、クライアント端末のローカルタイムゾーンに即して表示・集計しなければならない (SHALL)。

#### Scenario: グラフの日時表示とツールチップ
- **WHEN** 計算履歴推移グラフの X 軸目盛りまたはデータポイントのツールチップを表示するとき
- **THEN** 履歴の `created_at` が UTC としてパースされ、クライアント端末のローカルタイムゾーンに基づく日付および時刻が表示される。
