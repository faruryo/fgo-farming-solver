...
## Purpose
デイリー/ウィークリーミッションやイベント交換の期限切れによる取りこぼしを防ぐため、TODO の自動生成と期限間近の Web Push 通知を提供する。ここでは Web Push の環境ゲーティング（対応可否の検出）とユーザーガイダンスに関する要件を定義する。

<!-- NOTE: 本キャパビリティの基礎要件（自動TODO生成・許諾と登録・期限間近通知など）は
     openspec/changes/todo-management/specs/todo-notifications/spec.md に delta として存在し未sync。
     todo-management の archive 時にこの Requirements へマージされる想定。 -->
