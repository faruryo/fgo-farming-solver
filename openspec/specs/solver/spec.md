...
- **THEN** 画面下部に計算実行ボタンを含む Sticky Footer がセーフエリア余白を考慮して表示される。

#### Scenario: アイテム入力フィールドのレスポンシブ表示
- **WHEN** 画面幅が狭いモバイル端末（スマートフォン）でアイテム数入力フィールドを表示するとき
- **THEN** 各素材行はアイコン・素材名・入力欄が横一列に整列し、長い素材名でもレイアウトが崩れず入力できる。
- **THEN** ページ最下部の入力要素が固定フッターに隠れない余白が確保される。
